...
public class JPAService {
	...
	public List<Person> queryPersonByAge(Integer age) {
		...
		Query query = em.createNamedQuery("Person.findByAge") // NamedQuery 版
				.setParameter("ageValue", age);
		List<Person> list = query.getResultList();
		return list;
	}
	...
	public List<Person> findByName(String name) {
		return em.createNamedQuery("Person.findByName").setParameter("nameValue", name).getResultList();
	}

	// 查詢多筆 age 區間
	public List<Person> findByAgeBetween(Integer min, Integer max) {
		return em.createNamedQuery("Person.findByAgeBetween").setParameter("min", min).setParameter("max", max)
				.getResultList();
	}
